...
表示可能なASCII文字（`!` 〜 `~`）はそのまま保持し、それ以外のバイトはU+0100以降のUnicode文字に順番に割り当てます。
変換の具体例を示します。

```
0x20 (space) -> 'Ġ'   （表示可能ASCIIではないのでU+0120にマッピング）
0x21 (!)     -> '!'   （そのまま）
0x41 (A)     -> 'A'   （そのまま）
...
スペース (0x20) は表示可能なASCII範囲（`!`〜`~`）に含まれないため、独自のUnicode文字 `Ġ` にマッピングされます。
`Ġ` はBPE処理の内部でのみ使われる表現で、エンコード後のトークン文字列に現れる `Ġ` はすべて「先頭にスペースがある」ことを意味します。
...
スペースは後続の単語に吸収される形（`' world'` のように先頭のスペースを含む）で扱われます。
これがのちに `Ġ` として現れる元になります。
...
先ほどの `"Hello, world! It's a test."` を実際にエンコードすると次のようになります。

```
入力:   "Hello, world! It's a test."
IDs:    [15496, 11, 995, 0, 632, 338, 257, 1332, 13]
ピース: ['Hello', ',', 'Ġworld', '!', 'ĠIt', "'s", 'Ġa', 'Ġtest', '.']
```

`' world'`（先頭にスペース）がトークン `'Ġworld'` として表れています。
`Ġ` はスペースをバイトレベルで埋め込んだ表現であり、デコード時に元のスペースに戻ります。
