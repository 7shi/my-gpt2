...
GPT-2 の MLP は、768次元のベクトルを一度4倍の3072次元に拡張し、GELU 活性化を適用してから、元の768次元に戻します。
...
低次元空間では区別が難しい複雑なパターンも、高次元に引き上げると分離しやすくなります。たとえば2次元の平面上では分けられない点の集まりも、3次元に持ち上げれば平面で切り分けられることがあります。3072次元への拡張はこの原理の延長です。GELU で情報をフィルタリングしてから元の次元に圧縮することで、各トークンにとって重要な特徴だけを選択的に残しています。
...
ただし、この出力はそのまま次の層に渡されるのではなく、次回（👉[08](08_residual.md)）説明する残差接続によって元のベクトルに加算されます。
