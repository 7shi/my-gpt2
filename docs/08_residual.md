ページ：[1](01_overview.md) | [2](02_tokenizer.md) | [3](03_spiece.md) | [4](04_embedding.md) | [5](05_layer_norm.md) | [6](06_attention.md) | [7](07_mlp.md) | **8** | [9](09_output.md) | [10](10_kv_cache.md) | [11](11_architecture.md)
...
Attention も MLP も、基本演算は行列積と非線形関数の組み合わせであり、1958 年に提案されたパーセプトロン（`y = activation(W·x + b)`）の延長線上にあります。しかし素朴にパーセプトロンを何十層も積むと勾配消失で学習できません。残差接続が「バイパス道路」を作ることで深い層の積み重ねを可能にし、Transformer のスケーリングを支えています。
...
|---|---|---|
| 入力 | 0.2230 | |
| LayerNorm 1 後 | 0.1127 | |
| Attention 出力 | 1.2763 | |
| 残差接続 1 後 | 1.3099 | 入力 + Attention |
| LayerNorm 2 後 | 0.1560 | |
| MLP 出力 | 1.0851 | |
| 残差接続 2 後 | 2.0801 | 残差1 + MLP |
...
12層のブロックを通過した後、出力層（LM Head）に渡す前に最終 LayerNorm が適用されます。

```python
x = self.ln_f(x)
```

表の最終行を見ると、標準偏差が 14.2 → 7.95 に縮小しています。これは出力層に渡す前にスケールを整える役割です。cos 類似度は -0.04 → -0.003 とわずかに変化しています。

ここで注意が必要なのは、LayerNorm は通常の「ベクトルの正規化」（長さを1にする）とは異なるという点です。通常の正規化はベクトルの方向を変えないため cos 類似度に影響しませんが、LayerNorm は次のような操作を行います。

1. 平均を引く
2. 標準偏差で割る
3. 学習済みの gamma/beta で各成分を個別に変換する

特に 1. の平均の引き算と 3. の成分ごとのアフィン変換はベクトルの方向を変えるため、cos 類似度にも影響を与えます。この効果は以降のセクションで具体的に確認します。

## 4. 文脈付き埋め込み

同じ単語「bank」でも、周囲の文脈によって異なるベクトルに変化します。

文A: 'The river bank was covered'、文B: 'The money bank was covered'
...
ln_f を通すと類似度が 0.9326 → 0.9830 に上昇し、文脈による違いが縮小しています。セクション3で見た標準偏差の縮小と同様に、LayerNorm がベクトル間の方向の差を均一化する効果が現れています。
...
GPT-2 の最後のトークンのベクトルで文章間の cos 類似度を計算してみます。「The cat sat on the mat.」を基準に、他の9文と比較します。
...
期待する文が6位と、まったく機能していません。単語1つと文章ではトークン系列の長さが異なるため、隠れ状態のスケールに差が生じ、cos 類似度が歪みます。
...
正しい文が1位になりました（6位→1位）。文章間では順位の差を圧縮してしまう LayerNorm ですが、キーワード検索では系列長の違いによるスケール差を正規化する効果があります。ただし "animal" や "finance" では改善せず、意味的な検索としてはまだ不十分です。
