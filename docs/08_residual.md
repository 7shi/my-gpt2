...
現代の文章検索（セマンティック検索）や RAG で使われる「文章ベクトル」は、まさにこの Transformer の出力です。GPT-2 のようなモデルでは、最後のトークンの位置にあるベクトルが、それより前のすべての単語を Attention で「見ている」ため、文章全体の意味を要約した表現になっています。

ただし、GPT-2 は「次の単語の予測」に特化して学習されたモデルです。RAG などで使われる専用の埋め込みモデル（OpenAI の `text-embedding-3-small` や BERT 系モデルなど）は、「意味が似た文章をより近づけ、異なる文章をより遠ざける」ための追加の学習（**対照学習**）が行われているため、文章間の類似度がよりはっきりと分かれます。

## 実験：ブロック内外の表現変化

1ブロック内の各処理ステップの標準偏差、12ブロック全体を通したベクトルの変化、文脈による同一単語（bank）の表現の違いを確認します。実行結果は本文中で引用しています。
