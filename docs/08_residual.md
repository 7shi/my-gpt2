...
GPT-2 の推論は以下の流れで進みます。

1. トークン列
2. Embedding ([04](04_embedding.md))
3. Transformer Block × 12
4. LayerNorm ([05](05_layer_norm.md))
5. 出力層 ([09](09_output.md))

各 Transformer Block の内部は次の構造です。
...
`(+)` が**残差接続**で、各処理の出力を元の入力に加算する仕組みです。GPT-2 small ではこのブロックを12個積み重ねることで、テキストの高度な意味表現を獲得します。
...
最後の ln_f（最終 LayerNorm）を通すと、標準偏差が 14.2 → 7.95 に縮小しています。これは次の出力層（09 参照）に渡す前にスケールを整える役割です。cos 類似度は -0.04 → -0.003 とわずかに変化しています。
