...
### 実行方法
```bash
uv run experiments/17_output.py
```
...
1. **Embedding（12）**: トークンIDを768次元のベクトルに変換し、位置情報を加算する
2. **LayerNorm（13）**: 各処理の前にベクトルを正規化し、計算を安定させる
3. **Attention（14）**: 他のトークンとの関係を計算し、文脈を取り込む
4. **MLP（15）**: トークン単体の特徴を高次元空間で深掘りする
5. **残差接続（16）**: これらを組み合わせた Block を12層積み重ね、豊かな表現を獲得する
6. **出力（17）**: Weight Tying で語彙の確率分布に変換し、サンプリングで次の単語を選ぶ
