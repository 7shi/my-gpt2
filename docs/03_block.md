...
1つのBlockは「文脈理解（Attention）」と「単語の深掘り（MLP）」をセットで行い、残差接続でそれぞれの出力を元の入力に加算します。
...
**LayerNorm（層正規化）**とは、各トークンのベクトルを平均0・分散1に正規化してから、学習可能なスケール(γ)とシフト(β)を適用する処理です。バッチサイズやシーケンス長に依存しないため、GPT-2のように可変長の入力を扱うモデルで安定した学習が得られます。
...
MLPは、トークンごとに独立して計算を行います。Attentionが「トークン間の繋がり」を扱うのに対し、MLPは「トークン単体の意味の深掘り」を担当します。

GPT-2 small では、一度4倍の次元に拡張してから元の次元に戻します。

```
入力:   768次元
中間層: 3072次元  (4倍に拡張、GELUで活性化)
出力:   768次元  (元に戻す)
```

**GELU（Gaussian Error Linear Unit）**はReLUの滑らか版の活性化関数です。ReLUがx<0で完全にゼロにするのに対し、GELUは確率的に「ほぼゼロに近い値」にします。これにより勾配が0になるニューロンが減り、学習が安定します。
