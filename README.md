...
- `-t`, `--temperature`: サンプリング温度。値を大きくすると多様性が増し、0 に近づけると決定論的な生成になります（デフォルト: 1.0）。
...
- **NumPy 版** (`my_gpt2/`): Transformer の仕組みを理解するための教材。行列演算がすべて見える。
- **PyTorch 版** (`my_gpt2_torch/`): 同じロジックで高速に動作する実用版。GPU 対応も容易。
